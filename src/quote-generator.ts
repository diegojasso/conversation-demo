...
export class QuoteGenerator {
  public generateQuote(userInfo: UserInfo): QuoteResult {
    ...
    // Adjust for age
    if (userInfo.age < 25) riskFactor *= 1.5;
    else if (userInfo.age > 65) riskFactor *= 1.3;
    ...
    if (carAge > 10) riskFactor *= 1.2;

    const monthlyPremium = baseRate * riskFactor;
    let coverageLevel = "Standard";
    if (monthlyPremium < 75) coverageLevel = "Basic";
    if (monthlyPremium > 150) coverageLevel = "Premium";
